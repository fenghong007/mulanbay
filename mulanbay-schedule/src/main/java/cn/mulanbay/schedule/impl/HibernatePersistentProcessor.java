...
public class HibernatePersistentProcessor  extends BaseHibernateDao implements SchedulePersistentProcessor {
    ...
    public void updateTaskTriggerStatus(Long triggerId, TriggerStatus status) {
        try {
            String hql="update TaskTrigger set status=? where id=?";
            this.updateEntitys(hql,status,triggerId);
        } catch (BaseException e) {
    ...
    public List<TaskTrigger> getActiveTaskTrigger(String deployId, boolean supportDistri) {
        try {
            String hql="from TaskTrigger where triggerStatus=? ";
            if(supportDistri){
                hql+="and (deployId=? or distriable=1)";
            }else {
                hql+="and deployId=? ";
            }
    ...
    public boolean isTaskLogExit(Long taskTriggerId, Date bussDate) {
        try {
            String hql="select count(0) from TaskLog where taskTrigger.id=? and bussDate=? ";
    ...
    public boolean isTaskLogExit(String scheduleIdentityId) {
        try {
            String hql="select count(0) from TaskLog where scheduleIdentityId=?";
    ...
    public List<TaskLog> getAutoRedoTaskLogs(String deployId, boolean supportDistri, Date minDate) {
        ...
        try {
            ...
            sb.append("select tl from TaskLog tl,TaskTrigger tt where tl.taskTrigger.id = tt.id and tl.executeResult=? ");
            sb.append("and (tl.redoType=? or tl.redoType=?) ");
            if(supportDistri){
                sb.append("and (tt.deployId=? or tt.distriable=1) ");
            }else{
                sb.append("and tt.deployId=? ");
            }
            sb.append("and tl.startTime>=? ");
            sb.append("and tl.redoTimes < tt.allowedRedoTimes ");
            return this.getEntityListNoPageHQL(sb.toString(), JobExecuteResult.FAIL,
                    RedoType.AUTO_REDO,RedoType.ALL_REDO,deployId,minDate);
        } catch (BaseException e) {
