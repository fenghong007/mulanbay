...
var dataUrl='../bookCategory/getData';
...
function add() {
	...
	$('#ff').form('clear');
	initForm();
	$('#ff').form.url='../bookCategory/create';
}
...
function edit() {
	...
	var url='../bookCategory/get?id='+ rows[0].id;
	doAjax(null,url,'GET',false,function(data){
		$('#eidt-window').window('open');
        $('#ff').form('clear');
        initForm();
		$('#ff').form('load', data);
		$('#ff').form.url='../bookCategory/edit?id=' + data.id;
		//设置字符
...
function del() {
    var delUrlPrefix = '../bookCategory/delete';
