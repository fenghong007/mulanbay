...
import cn.mulanbay.pms.persistent.bean.SportExerciseDateStat;
import cn.mulanbay.pms.persistent.bean.SportExerciseMultiStat;
import cn.mulanbay.pms.persistent.bean.SportExerciseStat;
...
public class SportExerciseService  extends BaseHibernateDao {
    ...
    public Short getMaxOrderIndexOfSportMilestone(Integer sportTypeId,Long selfMilestoneId){
        try {
            String sql = "select max(order_index) from sport_milestone where sport_type_id = ? ";
    ...
    public List<SportMilestone> getAchieveMilestones(Long sportExerciseId){
        try {
            ...
            String hql="from SportMilestone where ((kilometres<=? and minutes=0) or (kilometres<=? and minutes>=? and minutes>0)) and sportType.id=? order by orderIndex desc";
    ...
    public SportMilestone getNextAchieveMilestone(Long sportExerciseId,NextMilestoneType type){
        try {
            ...
            if(null==type||type==NextMilestoneType.CURRENT){
                //当前该锻炼需要实现的下一个里程碑
                String hql="select max(orderIndex) from SportMilestone where ((kilometres<=? and minutes=0) or (kilometres<=? and minutes>=? and minutes>0)) and sportType.id=?";
                ...
                String hql2 ="from SportMilestone where orderIndex=? and sportType.id=?";
                return (SportMilestone) this.getEntityForOne(hql2,orderIndex,se.getSportType().getId());
            }else{
                //针对所有,未实现就可以了
                String hql="select max(orderIndex) from SportMilestone where sportExercise.id !=null and sportType.id=?";
    ...
    private void updateBest(SportExercise bean,boolean isCreate){
        try {
            ...
            String sql ="select max(kilometres) as maxKilometres,max(speed) as maxSpeed from sport_exercise where sport_type_id=? and exercise_date<? ";
            String addtion="";
            if(!isCreate){
            ...
            if(kilometres==null||kilometres.doubleValue()<bean.getKilometres().doubleValue()){
                //说明现在的是最佳,把以前的都更新为历史
                String hql="update SportExercise set mileageBest=? where mileageBest=? and sportType.id=?";
                this.updateEntitys(hql+addtion, BestType.ONCE,BestType.CURRENT,bean.getSportType().getId());
                needUpdate=true;
            ...
            if(speed==null||speed.doubleValue()<bean.getSpeed().doubleValue()){
                //说明现在的是最佳,把以前的都更新为历史
                String hql="update SportExercise set fastBest=? where fastBest=? and sportType.id=?";
                this.updateEntitys(hql+addtion, BestType.ONCE,BestType.CURRENT,bean.getSportType().getId());
                needUpdate=true;
    ...
    public void updateAndRefreshSportMilestone(boolean reInit,Integer sportTypeId){
        try {
            if(reInit){
                //全部初始化
                String sql ="update sport_milestone set sportExerciseId = null,fromExerciseDate = null,toExerciseDate=null,costDays=null where sportTypeId= ?";
                this.execSqlUpdate(sql,sportTypeId);
            }
            //获取没有完成的里程碑
            String hql="from SportMilestone where sportType.id =? and sportExercise.id is null order by orderIndex";
    ...
    public void updateAndRefreshSportExerciseMaxStat(Integer sportTypeId){
        try {
            //获取没有完成的里程碑
            String hql="from SportExercise where sportType.id =? order by exerciseDate";
    ...
    public void independUpdateAndRefreshSportMilestone(SportMilestone sm){
        try {
            String hh = "from SportExercise where sportType.id =? and kilometres>=?";
            if(sm.getMinutes()>0){
            ...
            if(se!=null){
                ...
                String sql ="update sport_milestone set sport_exercise_id = ?,from_exercise_date = ?,to_exercise_date=?,cost_days=? where id= ?";
    ...
    public Date getPreviousMilestoneSportExercise(SportMilestone milestone,Long sportExerciseId) {
        try {
            String hql = "from SportMilestone where sportType.id =? and sportExercise.id !=null and sportExercise.id !=? order by orderIndex desc";
            SportMilestone sm = (SportMilestone) this.getEntityForOne(hql, milestone.getSportType().getId(), sportExerciseId);
            if (sm == null) {
                //获取第一个
                String hh = "from SportExercise where sportType.id =? order by exerciseDate asc";
    ...
    public long getMilestoneCount(Long sportExerciseId){
        try {
            String hql="select count(*) from SportMilestone where sportExercise.id=?";
    ...
    public List<SportMilestone> getMilestoneList(Long sportExerciseId){
        try {
            String hql="from SportMilestone where sportExercise.id=?";
