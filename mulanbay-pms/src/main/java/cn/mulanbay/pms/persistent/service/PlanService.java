...
public class PlanService extends BaseHibernateDao {
    ...
    private static Logger logger = Logger.getLogger(PlanService.class);
    ...
    public void deletePlanReportData(PlanReportDataCleanSearch sf) {
        try {
            ...
            this.updateEntitys(hql, pr.getParameterValue());
    ...
    public void deletePlanConfig(Long id) {
        try {
            String hql = "delete from PlanConfigValue where planConfig.id=? ";
            this.updateEntitys(hql, id);

            String hql2 = "delete from PlanConfig where id=? ";
            this.updateEntitys(hql2, id);
     ...
     * @return
     */
    public Date getFirstStatDay(PlanConfig bean) {
        try {
            ...
            if(!StringUtil.isEmpty(userField)){
                ...
                hql+=" where "+userField+"="+bean.getUserId();
            }
    ...
    public void manualStat(PlanReportManualStat sf) {
        try {
            if (sf.getUserPlanId() == null) {
                ...
                sb.append("from UserPlan where status=? ");
                args.add(CommonStatus.ENABLE);
                if (sf.getPlanType() != null) {
                    sb.append("and planConfig.planType=? ");
     ...
     * @return
     */
    public PlanReportPlanCommend planCommend(PlanReportPlanCommendSearch sf) {
        try {
            PageRequest pr = sf.buildQuery();
            StringBuffer sb = new StringBuffer();
            ...
            List<PlanReportPlanCommend> list = this.getEntityListWithClassSQL(sb.toString(), pr.getPage(),
                    pr.getPageSize(), PlanReportPlanCommend.class, pr.getParameterValue());
            return list.get(0);
        } catch (BaseException e) {
            throw new PersistentException(ErrorCode.OBJECT_GET_LIST_ERROR,
                    "计划推荐异常", e);
    ...
    public void manualStat(UserPlan userPlan, Date startDate, Date endDate, Long userId, ManualStatType statType) {
        try {
            ...
            int bussDay = 0;
            String selectsql = "select count(0) from plan_report where userPlanId=? and userId=? and bussDay=?";
    ...
    public void reStatPlanReports(String ids,PlanReportReStatCompareType reStatCompareType,Integer compareYear){
        try {
            ...
            for(Long id :idArr){
                ...
                //
//                PlanConfigValue pcv = this.getPlanConfigValue(year,planReport.getUserPlan().getId(),reStatCompareType,planReport);
//                saveOrUpdatePlanReport(planReport.getUserPlan(),planReport.getBussDay(),planReport.getUserId(),pcv,planReport.getBussStatDate());
    ...
    private void saveOrUpdatePlanReport(UserPlan userPlan, int bussDay, Long userId, UserPlanConfigValue currentPcv, Date currentCaldate) {
        try {
            // 删除数据库里数据
            String deletesql = "delete from plan_report where user_plan_id=? and user_id=? and buss_day=?";
    ...
    public PlanReport statPlanReport(UserPlan userPlan, int bussDay, Long userId, UserPlanConfigValue userPlanConfigValue, Date currentCaldate,PlanReportDataStatFilterType filterType) {
        try {
            ...
            CommonSqlBean sqlBean = this.calSqlBean(userPlan,bussDay,userId,filterType);
     ...
     * @return
     */
    private CommonSqlBean calSqlBean(UserPlan userPlan, int bussDay, Long userId, PlanReportDataStatFilterType filterType){
        ...
        if(filterType==PlanReportDataStatFilterType.ORIGINAL||filterType==PlanReportDataStatFilterType.NO_USER){
            //添加时间过滤
            sb.append(" and "+getDateFilterString(planConfig.getDateField(),planConfig.getPlanType()));
            bean.addArg(bussDay);
        }
    ...
    private String getDateFilterString(String dateField,PlanType planType){
        if(planType==PlanType.YEAR){
            return "CAST(DATE_FORMAT("+dateField+",'%Y') AS signed)=?";
        }else if(planType==PlanType.MONTH){
            return "CAST(DATE_FORMAT("+dateField+",'%Y%m') AS signed)=?";
        }else if(planType==PlanType.DAY){
            return "CAST(DATE_FORMAT("+dateField+",'%Y%m%dd') AS signed)=?";
        }else {
    ...
    public UserPlanConfigValue getNearestUserPlanConfigValue(int year, Long userPlanId) {
        try {
            //取绝对值
            String sql="select abs(year-"+year+") as tt,t.id from user_plan_config_value t where t.user_plan_id=? order by tt asc, created_time desc ";
    ...
    public List<UserPlan> getUserPlanList(Long userId,String relatedBeans,PlanType planType) {
        try {
            ...
            args.add(userId);
            StringBuffer sb = new StringBuffer();
            sb.append("from UserPlan where userId=? ");
            if(!StringUtil.isEmpty(relatedBeans)){
                sb.append("and planConfig.relatedBeans=? ");
    ...
    public PlanReportTimeline getPlanReportTimeline(Long userPlanId, Date bussStatDate) {
        try {
            String hql="from PlanReportTimeline where userPlan.id=? and bussStatDate=? ";
    ...
    public List<PlanReportTimeline> getPlanReportTimelineList(Date startDate,Date endDate,Long userPlanId) {
        try {
            String hql="from PlanReportTimeline where bussStatDate>=? and bussStatDate<=? and userPlan.id=? order by bussStatDate";
    ...
    public PlanConfig getPlanConfig(Long id,Integer userLevel){
        try {
            String hql="from PlanConfig where id=? and level<=? ";
