...
import cn.mulanbay.pms.web.bean.response.chart.ChartCalandarMultiData;
...
public class SportExerciseController  extends BaseController {
    ...
    @Value("${system.maxHeartRate.base}")
    int baseMaxHeartRate;
    ...
    public String list() {
        return "sport/sportExerciseList";
    }
    ...
    @ResponseBody
    public ResultBean create(@Valid SportExerciseFormRequest formRequest) {
    ...
    @ResponseBody
    public ResultBean edit(@Valid SportExerciseFormRequest formRequest) {
    ...
    public String statList() {
        return "sport/sportExerciseDateStatList";
    }
    ...
    @ResponseBody
    public ResultBean dateStat(SportExerciseDateStatSearch sf) {
        ...
        if(sf.getDateGroupType()== DateGroupType.DAYCALENDAR){
            ChartCalandarData calandarData = ChartUtil.createChartCalandarData("锻炼统计","公里数","km",sf,list);
            if(!StringUtil.isEmpty(sf.getBestField())){
                ...
                for(SportExercise se : bests){
                    if(sf.getBestField().equals("mileageBest")){
        ...
        if(dateGroupType==DateGroupType.DAY){
            chartData.setLegendData(new String[]{"总公里数","锻炼时间","平均速度","配速","最大心率","平均心率","锻炼次数"});
        }else{
            //非天计算的则要添加平均公里数
            chartData.setLegendData(new String[]{"总公里数","平均公里数","锻炼时间","平均速度","配速","最大心率","平均心率","锻炼次数"});
        }
        ChartYData kilometresData =new ChartYData("总公里数");
        ChartYData averageKilometresData =new ChartYData("平均公里数");
        ...
        String totalString = totalCount.longValue()+"(次),"+totalKilometres.doubleValue()+"(公里)";
    ...
    public String yoyStatList() {
        return "sport/sportExerciseYoyStatList";
    }
     ...
     * @return
     */
    private ChartCalandarMultiData createChartCalandarMultiData(SportExerciseYoyStatSearch sf) {
