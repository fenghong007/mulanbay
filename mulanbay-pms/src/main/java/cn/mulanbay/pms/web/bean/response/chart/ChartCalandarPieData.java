 ...
 */
public class ChartCalandarPieData extends BaseChartData{
    ...
    private Map<String,List<ChartCalandarPieDetailData>> seriesData = new HashMap<>();
    ...
    public ChartCalandarPieData(UserBehaviorType behaviorType) {
    ...
    public Map<String, List<ChartCalandarPieDetailData>> getSeriesData() {
        return seriesData;
    }
     ...
     * @param days
     */
    public void addData(String date,String name,Object value,boolean dateRegion,int days){
        if(!dateRegion){
            this.addData(date,name,value);
        }else{
            ...
            for(int i=0;i<days;i++){
                ...
                //平均下去了，所以每个只是1
                this.addData(newDateString,name,1);
            }
        }
    }
     ...
     * @param value
     */
    public void addData(String date,String name,Object value){
        List<ChartCalandarPieDetailData> list = seriesData.get(date);
        if(list==null){
            list = new ArrayList<>();
        }
        ChartCalandarPieDetailData pdd =null;
        for(ChartCalandarPieDetailData dd :list){
        ...
        if(pdd==null){
            pdd = new ChartCalandarPieDetailData();
            pdd.setName(name);
            if (behaviorType == null) {
                //不同类型的值比较没有意义，因此全部设置为1
                pdd.setValue(1L);
